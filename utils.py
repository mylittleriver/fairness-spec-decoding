...
class GenerationMixin:
    ...
    def generate(
        ...
        negative_prompt_attention_mask: Optional[torch.Tensor] = None,
        **kwargs,
    ) -> Tuple[Union[GenerateOutput, torch.LongTensor], int, List[float], List[float], List[bool],List[int]]:
        r"""
        ...
        if generation_mode == GenerationMode.ASSISTED_GENERATION:
            ...
            result, n_matches_tot, p_list, q_list, ac_list, n_mat_list = self._assisted_decoding(
                input_ids,
        ...
        return result, n_matches_tot, p_list, q_list, ac_list, n_mat_list
    ...
    def _assisted_decoding(
        ...
        streamer: Optional["BaseStreamer"],
        **model_kwargs,
    ) -> Tuple[Union[GenerateNonBeamOutput, torch.LongTensor], int, List[float], List[float],List[bool], List[int]]:
        r"""
        ...
        **sample** (depending on `do_sample`), assisted by candidate sequences. Assisted generation is an example of a
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            ...
            # print("do_sample and candidate_logits is not None",do_sample and candidate_logits is not None)
            if do_sample and candidate_logits is not None:
                # print("do_sample and candidate_logits is not None")
                valid_tokens, n_matches, p_i, q_i, is_accepted= _speculative_sampling(
                    candidate_input_ids,
                    candidate_logits,
                    candidate_length,
                    new_logits,
            ...
            # print("n_matches in _assisted_decoding(): ", n_matches)
            # print(n_matches)
            # print(is_accepted)
            n_matches_tot+=n_matches
            # print("p_i:", p_i)
            # print("q_i:", q_i)
            p_list.extend(p_i.to('cpu').tolist())
            q_list.extend(q_i.to('cpu').tolist())
            # ac_list.extend(is_accepted.to('cpu').tolist())
            for tensor_item in is_accepted:
                ac_list.append(tensor_item.item())
            n_mat_list.append(n_matches)
        ...
        if return_dict_in_generate:
            if self.config.is_encoder_decoder:
                return GenerateEncoderDecoderOutput(
                    ...
                    decoder_hidden_states=decoder_hidden_states,
                    past_key_values=model_kwargs.get("past_key_values"),
                ), n_matches_tot, p_list, q_list, ac_list, n_mat_list
            else:
                return GenerateDecoderOnlyOutput(
                    sequences=input_ids,
                    scores=scores,
            ...
            return input_ids, n_matches_tot, p_list, q_list, ac_list, n_mat_list
...
def _speculative_sampling(
    ...
    min_length = min(q.shape[1], p.shape[1])
    q_aligned = q[:, :min_length, :]
    ...
    # filtered_is_accepted = []
    p_i_truncated = p_i
    q_i_truncated = q_i
    is_accepted_truncated = is_accepted
    if ((is_accepted == False).any().item()):
        for ind, item in enumerate(is_accepted):
            if not item:
                first_false_idx=ind
                break
        p_i_truncated = p_i[:first_false_idx + 1]
        q_i_truncated = q_i[:first_false_idx + 1]
        is_accepted_truncated = is_accepted[:first_false_idx + 1]
    ...
    if is_done_candidate and n_matches == candidate_length:
        ...
        if n_matches > 0:
            ...
            valid_tokens = t
    # print(n_matches)
    # print(is_accepted_truncated)

    return valid_tokens, n_matches, p_i_truncated, q_i_truncated, is_accepted_truncated
    # return valid_tokens, n_matches, p_i, q_i, is_accepted
